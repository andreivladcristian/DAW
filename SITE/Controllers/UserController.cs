...
namespace SITE.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult Index(int? page)
        {
            ...
            var list = db.tabelcategories.OrderByDescending(x => x.cat_id).ToList(); //ordoneaza ob descrescator
        ...
        [HttpPost]
        public ActionResult SignUp(tabeluser uvm, HttpPostedFileBase imgfile)
        {
        ...
        [HttpPost]
        public ActionResult login(tabeluser avm)
        {
            tabeluser ad = db.tabelusers.Where(x => x.u_email == avm.u_email && x.u_password == avm.u_password).SingleOrDefault();
            if (ad != null)
            {
        ...
        public ActionResult Ads(int? id, int? page)
        {
            int pagesize = 9, pageindex = 1;
            pageindex = page.HasValue ? Convert.ToInt32(page) : 1;
            var list = db.tabelproducts.Where(x => x.pro_fk_cat == id).OrderByDescending(x => x.pro_id).ToList();
            IPagedList<tabelproduct> stu = list.ToPagedList(pageindex, pagesize);
        ...
        public ActionResult Ads(int? id, int? page, string search)
        {
            int pagesize = 9, pageindex = 1;
            pageindex = page.HasValue ? Convert.ToInt32(page) : 1;
            var list = db.tabelproducts.Where(x => x.pro_name.Contains(search)).OrderByDescending(x => x.pro_id).ToList();
            IPagedList<tabelproduct> stu = list.ToPagedList(pageindex, pagesize);
